...
if( !class_exists( 'YITH_WC_Donations' ) ){
    ...
    class YITH_WC_Donations
    {
        ...
        public function action_links($links)
        {
            ...
            $links[] = '<a href="' . admin_url("admin.php?page={$this->_panel_page}") . '">' . __('Settings', 'yith-donations-for-woocommerce') . '</a>';
            $premium_live_text = defined( 'YWCDS_FREE_INIT' ) ?  __( 'Premium live demo', 'yith-donations-for-woocommerce' ) : __( 'Live demo', 'yith-donations-for-woocommerce' );

            $links[] = '<a href="'.$this->_premium_live_demo.'" target="_blank">'.$premium_live_text.'</a>';

            if ( defined( 'YWCDS_FREE_INIT' ) ) {
                  $links[] = '<a href="' . $this->get_premium_landing_uri() . '" target="_blank">' . __( 'Premium Version', 'yith-donations-for-woocommerce' ) . '</a>';
              }
        ...
        public function plugin_row_meta($plugin_meta, $plugin_file, $plugin_data, $status)
        {
             if ( ( defined( 'YWCDS_INIT' ) && ( YWCDS_INIT == $plugin_file ) ) ||
                 ( defined( 'YWCDS_FREE_INIT' ) && ( YWCDS_FREE_INIT == $plugin_file ) )
                    ...
                    $plugin_meta[] = '<a href="' . $this->_official_documentation . '" target="_blank">' . __( 'Plugin Documentation', 'yith-donations-for-woocommerce' ) . '</a>';
             }
        ...
        public function add_yith_donations_menu()
        {
            ...
            $args = array(
                ...
                'menu_title' => __('Donations', 'yith-donations-for-woocommerce'),
        ...
        public function add_free_frontend_style_script()
        {
            ...
            wp_enqueue_style('ywcds_style_free_frontend', YWCDS_ASSETS_URL . 'css/ywcds_free_frontend.css', array(), YWCDS_VERSION );
        ...
        private function _init_messages()
        {
            ...
            $messages = array(
                ...
                'already' => __('You have already added a donation to cart', 'yith-donations-for-woocommerce'),
                'success' => __('Thanks for your donation', 'yith-donations-for-woocommerce'),
                'text_button' => __( 'Add donation', 'yith-donations-for-woocommerce' ),
                'negative'  =>  __( 'Please enter a number >0', 'yith-donations-for-woocommerce' )
            );
            ...
            if ( !empty( $amount ) ) {
                ...
                if ( $amount != null && $amount*1 > 0 ) {
                   ...
                   if( $product_id != $this->_donation_id )
                    ...
                    wc_add_notice( $this->get_message('negative'),'error');
                    return 'negative';
                }
